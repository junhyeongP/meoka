...
public class RecipeController {
	...
	public String getWrite(RecipeVO recipe) throws Exception {

		// SELECT SEQ.NEXTVAL FROM DUAL
		// 1.RECIPEVO 객체 생성 setter로 recipeno 지정
		// 2.RECIPEDETAIL 객체 생성 (반복문)
		// 1,2 를 서비스로 전달
		System.out.println("*******************************도구테스트");
	 ...
	 * 
	 * int recipeno = service.write(recipe); recipeRequest.setRECIPENO(recipeno);
	 * List<RecipeDetailVO> list = service.getRecipeDetailList(recipeno);
	 * //ArrayList<RecipeDetailVO> rd = recipeRequest.getRecipeDetail();
	 * 
	 * service.deleteRecipeDetail(recipeno); int i = 1; for (RecipeDetailVO rd :
	 * recipeRequest.getRecipeDetail()) { rd.setRECIPENO(recipeno); i++;
	 * 
	 * service.writeRecipeDetail(rd); } service.modify(recipeRequest);
	 * 
