...
public  class RecipeServiceImpl implements RecipeService {
	 ...
	 * // 레시피 작성
	 * 
	 * @Override public int write(RecipeRequest recipeRequest) throws Exception {
	 * return dao.write(recipeRequest); }
	 */

	// 레시피 디테일 작성
	public void write(RecipeVO recipe) throws Exception {
		dao.write(recipe);
		RecipeDetailVO vo = new RecipeDetailVO();
		System.out.println("아오*******"+recipe);
		System.out.println("디테일넘버"+vo.getRECIPEDETAILNO());
		//recipedetial
		String[] detail = recipe.getRECIPEDETAIL();
		String[] photo = recipe.getPHOTO();
		int i = 0;
		for(String RECIPEDETAIL:detail){
			
			int RECIPEDETAILNO= i;
			String PHOTO = photo[i];
			RECIPEDETAIL=detail[i];
			System.out.println("-----------------");
			System.out.println(RECIPEDETAILNO);
			System.out.println(RECIPEDETAIL);
			System.out.println(PHOTO);
			dao.writeRecipeDetail(RECIPEDETAILNO,RECIPEDETAIL,PHOTO);
			i++;
		}
		
		//utensil
		String[] utensilName = recipe.getUtensilName();
		int j=0;
		for(String utensil:utensilName) {
			dao.writeUtensil(j,utensil);
	...
	@Override
	public void writeRecipeDetail(RecipeVO recipe) throws Exception {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void writeUtensil(RecipeVO recipe) throws Exception {
		// TODO Auto-generated method stub
